...
macro bench_par_matrix($name: ident, $t: expr, $n: expr, $m: expr) {
    #[bench]
    fn $name(bencher: &mut Bencher) {
        let mut rng = XorShiftRng::from_seed(SEED);
        let graph = AdjMatrix::gen_directed($n, $m, rng.sample_iter(&Standard));
        ...
        // > or parallel iterators will then operate within that threadpool.
        thread_pool.install(|| {
            bencher.iter(|| dfs::par_matrix(&graph));
        });
    }
}
...
bench_par_matrix!(par_matrix_t1_n4k_m400k, 1, 4_000, 400_000);
bench_par_matrix!(par_matrix_t2_n4k_m400k, 2, 4_000, 400_000);
bench_par_matrix!(par_matrix_t4_n4k_m400k, 4, 4_000, 400_000);
...
bench_seq!(seq_n40k_m4m, 40_000, 4_000_000);
bench_par!(par_t1_n40k_m4m, 1, 40_000, 4_000_000);
bench_par!(par_t2_n40k_m4m, 2, 40_000, 4_000_000);
bench_par!(par_t4_n40k_m4m, 4, 40_000, 4_000_000);

bench_par_matrix!(par_matrix_t1_n40k_m4m, 1, 40_000, 4_000_000);
bench_par_matrix!(par_matrix_t2_n40k_m4m, 2, 40_000, 4_000_000);
bench_par_matrix!(par_matrix_t4_n40k_m4m, 4, 40_000, 4_000_000);
